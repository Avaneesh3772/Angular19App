...
describe('RestatementGuard', () => {
  ...
  it('should allow access when user has restatement role', () => {
    appInitializerSpy.getAppConfigurationData.and.returnValue(userConfig(['restatement']));

    expect(guard.canMatch()).toBeTrue();
    expect(appCommonSpy.openNotAuthorizedDialogBox).not.toHaveBeenCalled();
  });

  it('should allow access when restatement is among multiple roles', () => {
    appInitializerSpy.getAppConfigurationData.and.returnValue(
      userConfig(['dashboard', 'templates', 'admin', 'restatement'])
    );
  ...
  it('should deny access and open dialog when user lacks restatement role', () => {
    appInitializerSpy.getAppConfigurationData.and.returnValue(
